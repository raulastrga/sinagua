...
function parsearTextoAJSON_2019(anio) {
    const carpetaDocs = path.join(__dirname, 'docs', anio);
    const carpetaJson = path.join(__dirname, 'json', anio); // Carpeta json/YYYY
    ...
    archivos.forEach(archivo => {
        ...
        if (datosJSON.length > 0) {
            // El archivo original es texto-DD-MM-YY.txt -> queremos datos-DD-MM-YY.json
